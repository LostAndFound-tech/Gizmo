...
- Logs the switch to RAG with timestamp
...
class SwitchHostTool(BaseTool):
    ...
    async def run(
        ...
        **kwargs,
        ...
        try:
            ...
            await asyncio.wait_for(
                wheel.run(
                    action="switch",
                    new_host=new_host,
                    staying_fronters=[f for f in fronters if f != new_host.lower()],
                ),
                timeout=3.0,  # hard 3 second timeout
            )
        ...
        # Build result summary for the agent
...
async def _log_switch_to_rag(
    ...
    """Ingest a brief note about this switch into relevant collections."""
    try:
        ...
        note = (
            f"On {timestamp}, {new_host} took over as host.{prev_note}{co_note}"
        )
        ...
        # Ingest into new host's collection, previous host's, and main
