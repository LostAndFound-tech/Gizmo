...
# ── Mid-conversation inference ────────────────────────────────────────────────

async def infer_from_exchange(
    ...
    known_headmates: list[str] = None,  # kept for back-compat, unused when resolved_subject provided
) -> int:
    """
    Called every 2 user messages during a live conversation.
    Asks the LLM what we can learn from the last few exchanges,
    then ingests each inference as an observation.

    resolved_subject: pre-computed note-taking subject from agent's sticky tracker.
    Falls back to current_host if not provided.
    """
    if not recent_messages:
        return 0

    subject = resolved_subject or current_host or "unknown"
    print(f"[Personality] infer_from_exchange — subject: {subject}")

    # ── Build a short transcript for the prompt ───────────────────────────────
    transcript = "\n".join(
        f"{'User' if m['role'] == 'user' else 'Gizmo'}: {m['content'][:300]}"
    ...
    prompt = [
        {
            "role": "user",
            "content": (
                f"Based on this conversation excerpt, what can we learn about {subject}?\n\n"
                f"{transcript}\n\n"
                f"Rules:\n"
                f"- Infer beyond explicit statements — what does their word choice, "
                f"questions, tone, or concerns reveal?\n"
                f"- Be specific. 'Seems to find social situations draining' beats 'might be introverted'.\n"
                f"- Only infer what is genuinely supported. Don't invent.\n"
                f"- Omit anything already obvious (e.g. 'the user sent a message').\n"
    ...
    try:
        ...
        raw = raw.strip().strip("```json").strip("```").strip()
        inferences: list[str] = json.loads(raw)

        if not isinstance(inferences, list):
            print(f"[Personality] infer_from_exchange — unexpected shape: {type(inferences)}")
            return 0
        ...
        for text in inferences:
            ...
            obs_store.ingest_texts(
                ...
                metadatas=[{
                    ...
                    "session_id": session_id,
                    "timestamp": now,
                    "source": "mid_conversation_inference",
                    "significance": "notable",
                    "weight": 0.0,
                    "recency_score": 1.0,
                }],
        ...
        print(f"[Personality] infer_from_exchange — {count} inferences stored for {subject}")
        return count

    except Exception as e:
        print(f"[Personality] infer_from_exchange() failed: {e}")
        return 0
...
async def retrieve_personality(
    ...
    # Interaction preferences — always injected when host is known.
    # These are verbatim, never synthesized, never filtered by relevance.
    if current_host:
        try:
            from core.interaction_prefs import format_prefs_for_prompt
            prefs_block = format_prefs_for_prompt(current_host)
            if prefs_block:
                sections.insert(0, prefs_block)  # top of personality block
        except Exception as e:
            print(f"[Personality] Interaction prefs retrieval failed: {e}")
