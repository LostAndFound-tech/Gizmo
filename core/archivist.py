...
from core.message_parser import parse_message, lore_as_facts
...
class Archivist:
    ...
    def receive(
        ...
        # ── Parse message — extract stage directions and lore ─────────────────
        # Must happen before anything else so classification, history, and
        # fact extraction all operate on the clean spoken text.
        parsed  = parse_message(message)
        message = parsed.clean   # strip markup — this is what's actually said
 
        if parsed.has_stage:
            log_event("Archivist", "STAGE_DIRECTIONS",
                session=session_id[:8],
                stage=parsed.stage,
            )
        if parsed.has_lore:
            log_event("Archivist", "LORE_RECEIVED",
                session=session_id[:8],
                lore=parsed.lore,
            )
        ...
        log_event(
         ...
         # ── Store lore as facts immediately ───────────────────────────────────
        # Lore is intentional world-building — store it so Gizmo can recall it.
        # Stage directions are NOT stored — they're transient room context.
        if parsed.has_lore and headmate:
            try:
                from core.llm import llm as _llm
                from memory.memory_writer import extract_and_store
                lore_text = " ".join(parsed.lore)
                ...
                log_error("Archivist", "lore ingestion failed", exc=e)
