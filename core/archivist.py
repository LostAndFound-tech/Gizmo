...
LLM calls are delegated to Observer and InferenceEngine.
"""
...
class Brief:
    ...
    interrupt:            Optional[str] = None  # wait request — surfaces before processing
...
def _load_headmate_persona(headmate: str) -> str:
    """
    Load the persona field from the headmate's JSON file.
    Returns empty string if not set.
    Passed to infer_from_exchange so inference is shaped by who this person is.
    """
    if not headmate:
        return ""
    try:
        ...
        # Also include explicit instructions
...
async def _run_inference(
    ...
    """
    Ask the LLM what we can learn about the resolved_subject from this exchange.
    Writes inferences to memory collection with type: observation.
    Passes headmate persona so the inference understands who this person is.
    """
...
class Archivist:
    ...
    def _resolve_inference_subject(
        ...
        """
        Determine who the current exchange is about for inference purposes.
        Sticky — once a headmate is named they stay the subject until
        current_host is named or a different headmate is named.
        """
        ...
        # If speaker is named explicitly → reset to host
        if speaker and speaker in msg_lower:
            current_subject = current_host or ""

        # If another headmate is named → they become the subject
    ...
    def receive(
        ...
        # ── Intent wiring — inbound name check / wait request ─────────────────
        try:
            from core.intent_wiring import intent_wiring
            _interrupt = intent_wiring.on_inbound(
                message=message,
                headmate=headmate,
                session_id=session_id,
            )
            if _interrupt:
                brief.interrupt = _interrupt
        except Exception as e:
            log_error("Archivist", "intent_wiring.on_inbound failed", exc=e)
    ...
    def receive_outgoing(
        ...
        try:
            ...
            # Ask LLM what we can learn beyond explicit facts.
            # Uses sticky subject tracking + headmate persona for context.
            self._user_msg_counts[session_id] = (
                self._user_msg_counts.get(session_id, 0) + 1
            )

            if self._user_msg_counts[session_id] % 2 == 0:
                # Resolve subject — sticky, pronoun-safe
                try:
                ...
                # Get recent messages for inference context
                try:
        ...
        # ── Intent wiring — event extraction, people touch, name scan ─────────
        try:
            from core.intent_wiring import intent_wiring
            from core.session_manager import session_manager as _sm2
            _state        = _sm2._sessions.get(session_id)
            _session_file = (
                _state.session_file
                if _state and hasattr(_state, "session_file")
                else session_id
            )
            ...
            log_error("Archivist", "intent_wiring.on_outgoing failed", exc=e)
