...
Embeddings use AWS Bedrock Titan Embeddings V2 instead of
sentence-transformers — no torch dependency, pay per token,
works cleanly in Lambda.
...
class BedrockTitanEmbeddingFunction(EmbeddingFunction):
    """
    ChromaDB-compatible embedding function using AWS Bedrock Titan V2.
    Replaces SentenceTransformerEmbeddingFunction — no torch required.
    Titan V2 produces 1024-dimensional embeddings.
    ...
    def __call__(self, input: Documents) -> Embeddings:
        """
        Embed a list of documents. Called by ChromaDB automatically.
        Batches calls to Bedrock — one request per document.
        """
        ...
        for text in input:
            try:
                ...
                print(f"[RAG] Embedding failed for text '{text[:50]}': {e}")
                # Return zero vector on failure so we don't crash the whole batch
...
class RAGStore:
    def __init__(
        ...
        self.embed_fn = BedrockTitanEmbeddingFunction()
