...
class SessionManager:
    ...
    async def _close_session(self, session_id: str, state: SessionState) -> None:
        ...
        # ── Intent wiring — temperature reset ─────────────────────────────────
        try:
            from core.intent_wiring import intent_wiring
            for host in state.hosts:
                intent_wiring.on_session_close(headmate=host)
        except Exception as e:
            log("SessionManager", f"intent_wiring.on_session_close failed: {e}")

        # ── Psych processor — queue daily analysis ────────────────────────────
        try:
            from core.psych_processor import psych_processor
            import asyncio as _asyncio
            for host in state.hosts:
                _asyncio.ensure_future(
                    psych_processor.run_for(headmate=host)
                )
        except Exception as e:
