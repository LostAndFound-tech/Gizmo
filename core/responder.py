...
- Pulls relevant knowledge entries by vocabulary tag match
...
async def _assemble_brief(
    ...
    try:
        ...
        if vocabulary and user_message.strip():
            matched_tags = await get_relevant_tags(user_message, vocabulary)
            if matched_tags:
                speaker = fronters[0] if fronters else None
                knowledge = librarian.get_knowledge(tags=matched_tags, speaker=speaker)
                if knowledge:
                    lines = []
                    for e in knowledge:
                        line = f"- {e['fact']}"
...
- What you know about their world (places, objects, events, routines)
